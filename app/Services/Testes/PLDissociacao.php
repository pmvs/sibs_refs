...
class PLDissociacao extends Controller implements PLInterface
{
    ...
    */ 
    public function __construct() 
    {
        ...
        \Log::channel($this->logchannel)->info('__construct PLDissociacao');
    }
    ...
    public function __construct_3($logchannel, $tabNumber, $testNumber) 
    {
        ...
        \Log::channel($this->logchannel)->info('__construct_3 PLDissociacao');
    }
    ...
    public function efetuaPedido($fields )
    {
        try {
            ...
            \Log::channel($this->logchannel)->info('efetuaPedido em PLDissociacao');
            \Log::channel($this->logchannel)->info('fillData em PLDissociacao...');
           
            $myJSON = $this->fillData($fields);
            if ( ! $myJSON ) {
                \Log::channel($this->logchannel)->warning('fillData em PLDissociacao...NOT OK');
                return 'Erro: Pedido de execução de teste inválido -> fillData em PLDissociacao';
            }
            \Log::channel($this->logchannel)->info('fillData em PLDissociacao...OK');

            \Log::channel($this->logchannel)->info('executaTeste em PLDissociacao');
            \Log::channel($this->logchannel)->info($myJSON);
            $response = $this->executaTeste($fields, $myJSON);
            if ( ! $response ) {
                \Log::channel($this->logchannel)->warning('executaTeste em PLDissociacao...NOT OK');
                return 'Erro: Pedido de execução de teste inválido -> executaTeste em PLDissociacao';
            }
            \Log::channel($this->logchannel)->info('executaTeste em PLDissociacao...OK');
    ...
    public function fillData( $fields )
    {
        try {
            ...
            \Log::channel($this->logchannel)->info('fillData em PLDissociacao...teste nr ' . $fields['testNumber'] );
        ...
        try {
            ...
            if (array_key_exists('pspCode', $fields)) {
                if ( ! empty($fields['pspCode'])) {
                    ...
                    \Log::channel($this->logchannel )->info('psp_code not set ');
                    return null;
                }
            }else{
                \Log::channel($this->logchannel )->info('psp_code not set ');
    ...
    private function executaTeste($fields, $myJSON )
    {
        try{
            ...
            \Log::channel($this->logchannel)->info('PLDissociacao : executa teste ');
           // $this->token_valido = config('app.access_token_valido');
            ...
            try {
                ...
                if ( $httpCode == '200' ) {
                    ...
                    \Log::channel($this->logchannel)->info('Vai efetuar a dissociação na base de dados bp_pl_assoc...'); 

                    //get active record association
                    $correlation_id_ativo = $oauth2->getActiveCorrelationIdFromDB($contrato,$myJSON);
                    if ( $correlation_id_ativo != '' ) {
                        $oauth2->updateDissociacaoToDB( $correlation_id , $correlation_id_ativo);
                    }else{
                        \Log::channel($this->logchannel)->warning('Correlation id activo na base de dados bp_pl_assoc...Não encontrado'); 
                    }
