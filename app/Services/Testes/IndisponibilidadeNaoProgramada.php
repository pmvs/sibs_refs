...
class IndisponibilidadeNaoProgramada extends Controller implements IndisponibilidadeInterface
{
    ...
    */ 
    public function __construct() 
    {
        ...
        \Log::channel($this->logchannel)->info('__construct IndisponibilidadeNaoProgramada');
    }
    ...
    public function __construct_3($logchannel, $tabNumber, $testNumber) 
    {
        ...
        \Log::channel($this->logchannel)->info('__construct_3 IndisponibilidadeNaoProgramada');
    }
    ...
    public function efetuaPedido($fields )
    {
        try {
            ...
            \Log::channel($this->logchannel)->info('efetuaPedido em IndisponibilidadeNaoProgramada');
            \Log::channel($this->logchannel)->info('fillData em IndisponibilidadeNaoProgramada...');
           
            $myJSON = $this->fillData($fields);
            if ( ! $myJSON ) {
                \Log::channel($this->logchannel)->warning('fillData em IndisponibilidadeNaoProgramada...NOT OK');
                return 'Erro: Pedido de execução de teste inválido -> fillData em IndisponibilidadeNaoProgramada';
            }
            \Log::channel($this->logchannel)->info('fillData em IndisponibilidadeNaoProgramada...OK');

            \Log::channel($this->logchannel)->info('executaTeste em IndisponibilidadeNaoProgramada');
            \Log::channel($this->logchannel)->info($myJSON);
            $response = $this->executaTeste($fields, $myJSON);
            if ( ! $response ) {
                \Log::channel($this->logchannel)->warning('executaTeste em IndisponibilidadeNaoProgramada...NOT OK');
                return 'Erro: Pedido de execução de teste inválido -> executaTeste em IndisponibilidadeNaoProgramada';
            }
            \Log::channel($this->logchannel)->info('executaTeste em IndisponibilidadeNaoProgramada...OK');
    ...
    public function fillData( $fields )
    {
        try {
            ...
            \Log::channel($this->logchannel)->info('fillData em IndisponibilidadeNaoProgramada...teste nr ' . $fields['testNumber'] );
    ...
    private function createTeste2( $fields )
    {
        try {
            ...
            if (array_key_exists('is_finished', $fields)) {
                if ( $fields['is_finished'] == 'false' ){
                    Log::channel($this->logchannel )->error('is_finished: FALSE');
                   // $myObj->{'is_in_progress'} =  (bool) $fields['is_in_progress'];
                    $myObj->{'is_finished'} = (bool) false;
                }else {
                    Log::channel($this->logchannel )->error('is_finished: TRUE');
                    $myObj->{'is_finished'} = (bool) true;// Returns true
                }        
                
            }else {
                \Log::channel($this->logchannel )->error('is_finished doesnt exist. is mandatory. return null');
            }
    ...
    private function createTeste3( $fields )
    {
        try {
            ...
            $myObj->{'timestamp'} = $this->getTimestampFuture();
    ...
    private function createTeste4( $fields )
    {
        try {
            ...
            \Log::channel($this->logchannel )->info('createTeste4 with ' . print_r( $fields , true));
            ...
            $myObj->{'timestamp'} = $this->getTimestampPast();
        ...
        try{
           ...
           // $this->token_valido = config('app.access_token_valido');
            ...
            switch ( $fields['testNumber'] ){
                ...
                case 20: 
                case 21: 
                    $audience = config('enums.apibp_dev.endpoints_plcp.INDISPONIBILIDADE_NAO_PROGRAMADA_CREATE');
                    if ( config('app.env') == 'prod' ) {
                        $audience = config('enums.apibp.endpoints_plcp.INDISPONIBILIDADE_NAO_PROGRAMADA_CREATE');
                    }
                ...
                case 18: 
                case 22: 
                    $audience = config('enums.apibp_dev.endpoints_plcp.INDISPONIBILIDADE_NAO_PROGRAMADA_UPDATE');
                    if ( config('app.env') == 'prod' ) {
                        $audience = config('enums.apibp.endpoints_plcp.INDISPONIBILIDADE_NAO_PROGRAMADA_UPDATE');
                    }
                    \Log::channel($this->logchannel)->info(  'Audience ...' . $audience);
                    if (  $audience == '' ) {
                        \Log::channel($this->logchannel)->warning('Não foi possivel obter o url de destino');
