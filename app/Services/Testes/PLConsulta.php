...
class PLConsulta extends Controller implements PLInterface
{
    ...
    */ 
    public function __construct() 
    {
        ...
        \Log::channel($this->logchannel)->info('__construct PLConsulta');
    }
    ...
    public function __construct_3($logchannel, $tabNumber, $testNumber) 
    {
        ...
        \Log::channel($this->logchannel)->info('__construct_3 PLConsulta');
    }
    ...
    public function efetuaPedido($fields )
    {
        try {
            ...
            \Log::channel($this->logchannel)->info('efetuaPedido em PLConsulta');
            \Log::channel($this->logchannel)->info('fillData em PLConsulta...');
           
            $myJSON = $this->fillData($fields);
            if ( ! $myJSON ) {
                \Log::channel($this->logchannel)->warning('fillData em PLConsulta...NOT OK');
                return 'Erro: Pedido de execução de teste inválido -> fillData em PLConsulta';
            }
            \Log::channel($this->logchannel)->info('fillData em PLConsulta...OK');

            \Log::channel($this->logchannel)->info('executaTeste em PLConsulta');
            \Log::channel($this->logchannel)->info($myJSON);
            $response = $this->executaTeste($fields, $myJSON);
            if ( ! $response ) {
                \Log::channel($this->logchannel)->warning('executaTeste em PLConsulta...NOT OK');
                return 'Erro: Pedido de execução de teste inválido -> executaTeste em PLConsulta';
            }
            \Log::channel($this->logchannel)->info('executaTeste em PLConsulta...OK');
    ...
    public function fillData( $fields )
    {
        try {
            ...
            \Log::channel($this->logchannel)->info('fillData em PLConsulta...teste nr ' . $fields['testNumber'] );
    ...
    private function createTeste1( $fields )
    {
        try {
            ...
            $phone_book = array_map('trim', explode(',', $fields['iban']));
            ...
            if ( count($phone_book) > 0 ) {
                ...
                \Log::channel($this->logchannel )->info('phone_book not set');
                return null;
            }
     ...
     private function createTeste3( $fields )
    {
        try {
            ...
            $phone_book = array_map('trim', explode(',', $fields['nif']));
