...
class ChargeRequestPlContact implements ShouldQueue
{
     ...
     */
    public function __construct( $iban, $i )
    {
    ...
    {
        \Log::channel($this->logchannel)->info('JOB ChargeRequestPlContact HANDLE ' . $this->counter .  ' ' . $this->iban);

        //\Log::channel($this->logchannel)->info( 'Vai invocar o servico do BP para obter nome COp do ...' .  $this->iban);
        $start_main = microtime(true);

        $content = ( new BancoPortugalService( $this->logchannel ) )->getNomePrimeiroTitular($this->contrato,  $this->iban);
        // if ( $content ) {
    ...
    public function failed()
    {
        ...
        $datainfo = 'ChargeRequestPlContact JOB failed';
